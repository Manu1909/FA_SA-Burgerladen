...
public class Controller {
	...
	private static int [] risikoEingetreten = new int [10]; //Array, das einspeichert, wenn ein bestimmtes Ereignis f�r einen Spieler aufgetreten ist
	...
	public static void startGame(){
		...
		try {
			while (neuerSpieler){
				...
				//LieferantenBestellung
				System.out.println("\nBitte wählen sie im Folgenden, f�r wie viele Burger sie Zutaten in dieser Periode kaufen wollen");
	...
	public static String [] ereignisTrittEin(int parameter) {
		...
		int [] initErgebnis = new int [unternehmen.size()]; //Pr�ft f�r jedes Unternehmen, ob es sich um das erste Ereignis handelt
		int checkArray = 0; //gibt den Inhalt von initErgebnis in Form einer Variable wieder
		
		if (parameter == 1){ //Bestimmter Eintritt eines Ereignisses f�r den SpielablaufTest
			//Gammelfleischskandal f�r Runde 6
			for (int i = 0; i < unternehmen.size(); i++) {
			...
			for (int i = 0; i < Datenbank.fl.length; i++) {
					int zufallszahl = (int)(Math.random() * 100) + 1; //Abh�ngig von der generierten Zufallszahl findet ein Ereignis statt
			...
			for (int i = 0; i < unternehmen.size(); i++) {
				int zufallszahl = (int)(Math.random() * 100) + 1; //Zufallszahl f�r die beiden anderen Ereignissen
